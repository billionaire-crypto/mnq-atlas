...
def test_v2_evidence_pin_restorations_bind_the_immutable_prior_entries():
    ...
    assert set(run_commit["evidence_hashes"]) == set(
        predecessor_run["evidence_hashes"]
    ) | {predecessor_run_path}
    ...
    original = altered["evidence_hashes"][predecessor_run_path]
    altered["evidence_hashes"][predecessor_run_path] = (
        "0" if original[0] != "0" else "1"
    ) + original[1:]
    assert _mismatched_evidence(altered) == [predecessor_run_path]
...
def test_v2_restorations_prevent_undocumented_evidence_withdrawal(monkeypatch):
    ...
    _assert_evidence_superset(effective_run, predecessor_run)
    ...
    assert len(effective_run["evidence_hashes"]) == 24
