...
V2_DIRECT_PIN_RUN_COMMIT_AUDIT_ENTRY = (
    REPO_ROOT
    / "mnq_lab/ledger/audit_entries"
    / "2026-08-12-phase7-unit-o-session-aware-v2-run-commit-direct-pin-restoration.json"
)
V2_PREDECESSOR_RUN_COMMIT_AUDIT_ENTRY = (
    REPO_ROOT
    / "mnq_lab/ledger/audit_entries"
    / "2026-08-13-phase7-unit-o-session-aware-v2-run-commit-predecessor-pin-restoration.json"
)
V2_D38_EVIDENCE_PIN_AMENDMENT = (
    REPO_ROOT
    / "mnq_lab/ledger/audit_entries"
    / "2026-08-14-phase7-unit-o-session-aware-v2-d38-evidence-pin-amendment.json"
)
V2_D39_EVIDENCE_PIN_AMENDMENT = (
    REPO_ROOT
    / "mnq_lab/ledger/audit_entries"
    / "2026-08-15-phase7-unit-o-session-aware-v2-d39-evidence-pin-amendment.json"
)
V2_D40_EVIDENCE_PIN_AMENDMENT = (
    REPO_ROOT
    / "mnq_lab/ledger/audit_entries"
    / "2026-08-15-phase7-unit-o-session-aware-v2-d40-evidence-pin-amendment.json"
)
V2_D41_EVIDENCE_PIN_AMENDMENT = (
    REPO_ROOT
    / "mnq_lab/ledger/audit_entries"
    / "2026-08-15-phase7-unit-o-session-aware-v2-d41-evidence-pin-amendment.json"
)
V2_RUN_COMMIT_AUDIT_ENTRY = (
    REPO_ROOT
    / "mnq_lab/ledger/audit_entries"
    / "2026-08-15-phase7-unit-o-session-aware-v2-d42-evidence-pin-amendment.json"
)
...
def _v2_run_commit_audit_entry() -> dict:
    return _v2_entry(
        "2026-08-15-phase7-unit-o-session-aware-v2-d42-evidence-pin-amendment"
    )
...
def test_negative_case_a_tampered_evidence_hash_is_detected():
    ...
    entry = copy.deepcopy(_v2_run_commit_audit_entry())
...
def test_v2_evidence_pin_restorations_bind_the_immutable_prior_entries():
    ...
    direct_pin_run = _v2_entry(
        "2026-08-12-phase7-unit-o-session-aware-v2-run-commit-direct-pin-restoration"
    )
    predecessor_run = _v2_entry(
        "2026-08-13-phase7-unit-o-session-aware-v2-run-commit-predecessor-pin-restoration"
    )
    d38_amendment = _v2_entry(
        "2026-08-14-phase7-unit-o-session-aware-v2-d38-evidence-pin-amendment"
    )
    d39_amendment = _v2_entry(
        "2026-08-15-phase7-unit-o-session-aware-v2-d39-evidence-pin-amendment"
    )
    d40_amendment = _v2_entry(
        "2026-08-15-phase7-unit-o-session-aware-v2-d40-evidence-pin-amendment"
    )
    d41_amendment = _v2_entry(
        "2026-08-15-phase7-unit-o-session-aware-v2-d41-evidence-pin-amendment"
    )
    ...
    restored_run_path = V2_RESTORED_RUN_COMMIT_AUDIT_ENTRY.relative_to(
        REPO_ROOT
    ).as_posix()
    direct_pin_run_path = V2_DIRECT_PIN_RUN_COMMIT_AUDIT_ENTRY.relative_to(
        REPO_ROOT
    ).as_posix()
    predecessor_run_path = V2_PREDECESSOR_RUN_COMMIT_AUDIT_ENTRY.relative_to(
        REPO_ROOT
    ).as_posix()
    d38_amendment_path = V2_D38_EVIDENCE_PIN_AMENDMENT.relative_to(
        REPO_ROOT
    ).as_posix()
    d39_amendment_path = V2_D39_EVIDENCE_PIN_AMENDMENT.relative_to(
        REPO_ROOT
    ).as_posix()
    d40_amendment_path = V2_D40_EVIDENCE_PIN_AMENDMENT.relative_to(
        REPO_ROOT
    ).as_posix()
    d41_amendment_path = V2_D41_EVIDENCE_PIN_AMENDMENT.relative_to(
        REPO_ROOT
    ).as_posix()
    ...
    assert set(run_commit["evidence_hashes"]) == set(
        d41_amendment["evidence_hashes"]
    ) | {d41_amendment_path}
    for relative, digest in direct_pin_run["evidence_hashes"].items():
        if relative == "docs/DISCREPANCIES.md":
    ...
    assert run_commit["evidence_hashes"][restored_run_path] == hashlib.sha256(
        V2_RESTORED_RUN_COMMIT_AUDIT_ENTRY.read_bytes()
    ).hexdigest()
    assert run_commit["evidence_hashes"][direct_pin_run_path] == hashlib.sha256(
        V2_DIRECT_PIN_RUN_COMMIT_AUDIT_ENTRY.read_bytes()
    ).hexdigest()
    assert run_commit["evidence_hashes"][predecessor_run_path] == hashlib.sha256(
        V2_PREDECESSOR_RUN_COMMIT_AUDIT_ENTRY.read_bytes()
    ).hexdigest()
    assert run_commit["evidence_hashes"][d38_amendment_path] == hashlib.sha256(
        V2_D38_EVIDENCE_PIN_AMENDMENT.read_bytes()
    ).hexdigest()
    assert run_commit["evidence_hashes"][d39_amendment_path] == hashlib.sha256(
        V2_D39_EVIDENCE_PIN_AMENDMENT.read_bytes()
    ).hexdigest()
    assert run_commit["evidence_hashes"][d40_amendment_path] == hashlib.sha256(
        V2_D40_EVIDENCE_PIN_AMENDMENT.read_bytes()
    ).hexdigest()
    assert run_commit["evidence_hashes"][d41_amendment_path] == hashlib.sha256(
        V2_D41_EVIDENCE_PIN_AMENDMENT.read_bytes()
    ).hexdigest()

    # Negative witness: a schema-valid restoration with a corrupted prior-entry
    # pin is detected by repository-byte re-reading.
    altered = copy.deepcopy(run_commit)
    original = altered["evidence_hashes"][d41_amendment_path]
    altered["evidence_hashes"][d41_amendment_path] = (
        "0" if original[0] != "0" else "1"
    ) + original[1:]
    assert _mismatched_evidence(altered) == [d41_amendment_path]
...
def test_v2_restorations_prevent_undocumented_evidence_withdrawal(monkeypatch):
    ...
    _assert_evidence_superset(direct_pin_run, restored_run)
    _assert_evidence_superset(predecessor_run, direct_pin_run)
    _assert_evidence_superset(d38_amendment, predecessor_run)
    _assert_evidence_superset(d39_amendment, d38_amendment)
    _assert_evidence_superset(d40_amendment, d39_amendment)
    _assert_evidence_superset(d41_amendment, d40_amendment)
    _assert_evidence_superset(effective_run, d41_amendment)
    ...
    assert len(direct_pin_run["evidence_hashes"]) == 21
    assert len(effective_run["evidence_hashes"]) == 28
    ...
    # Decisive regression witness for the dffe3a5 gap: the 2026-08-10 prior
    # cannot see any of the six pins, but the original run entry sees all six.
    receipt_paths = tuple(
        sorted(
            relative
            for relative in prior_run["evidence_hashes"]
            if relative.startswith(
                "C:/Users/kyawz/mnq_atlas_runs/"
                "phase7-unit-o-v2-final-6f1c506-attempt-2/"
            )
        )
    )
    assert len(receipt_paths) == 6
    historical_gap = copy.deepcopy(restored_run)
    for relative in receipt_paths:
        del historical_gap["evidence_hashes"][relative]
    ratification._validate_audit_entry(historical_gap)
    _assert_evidence_superset(historical_gap, amended_run)

    all_deleted = copy.deepcopy(effective_run)
    for relative in receipt_paths:
        del all_deleted["evidence_hashes"][relative]
    ratification._validate_audit_entry(all_deleted)
    with pytest.raises(AssertionError) as all_exc_info:
        _assert_evidence_superset(all_deleted, prior_run)
    for relative in receipt_paths:
        assert relative in str(all_exc_info.value)
    assert _mismatched_evidence(all_deleted) == []
